...
export default function Settings() {
  ...
  const saveWatermark = async () => {
    ...
    await callCloud('set_setting', { watermark: wm });
  };
  ...
  return (
    <View className="page">
      ...
      {/* AI 接口 */}
      <View className="setting-card" onClick={() => setShowAi(true)}>
        <View className="setting-row">
          <Text className="setting-label">AI 接口</Text>
          <View className="setting-value">
            <Text className="sv-text">自定义 API</Text>
      ...
      <Popup show={showAi} position="bottom" onClose={() => setShowAi(false)}>
        <View className="ed-bar">
          <Text className="ed-title kai">AI 接口</Text>
        ...
        <View className="ed-foot">
          <Button className="d-btn" loading={aiBusy} onClick={saveAi}>
            保存
          </Button>
        </View>
      </Popup>
